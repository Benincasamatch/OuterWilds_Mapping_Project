...
| 项 | 标准 | 验证方式 |
|---|---|---|
| 核心交互 | 桌面为前台时，拖动旋转 + 滚轮缩放均生效 | `spike/input-probe/` 探针，实测截图 |
| 挂机开销 | 空闲（**无输入 且 无活跃事件**）CPU < 3%、GPU < 5%（**实测对象**：`webwallpaper64.exe` 及其 CEF 子进程树 + `wallpaperui.exe`；单显示器 2560×1600；连续 10 分钟采样） | 任务管理器（含子进程）/ PresentMon，日志留档 |
