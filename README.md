...
- 浏览器及 WE 预览窗口的拖动/缩放已有验证。2026-09-19 横向桌面实测拖动通过，**滚轮未收到**；使用右下角缩放按钮作为备用，不将其视作原滚轮要求已通过。
...
1. 旧桌面测试缺少持续运行证据。2026-09-19 新实测确认横向桌面拖动通过、滚轮未收到；竖屏输入仍需单独验证。
2. 已有真实桌面长时参考数据和双屏短测，但空闲组中断、活跃组差0.83秒且一个GPU样本缺失；指定2560×1600及多屏长时验收仍未完成。
